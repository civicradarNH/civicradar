...
async def run_citizen_tests(s: Suite, browser):
    ...
    native_share_ok = await page.evaluate("""() => {
      ...
      const prompt = document.querySelector('#successModal .success-share-prompt');
      return !btn && !prompt;
    }""")

    s.record('C17b', 'Citizen', 'Success More has no native Share / share prompt', native_share_ok)
...
async def run_remaining_scenarios(s: Suite, browser):
    ...
    # v392+: standalone ToS gate merged into onboarding step 2 — wipe reopens onboarding.
    s.record('P11', 'Profile', 'Delete my data resets to onboarding', await is_open(page, 'onboardingOverlay'))
...
async def run_extended_scenarios(s: Suite, browser):
    ...
    s.record('DF02', 'Differentiation', 'About disclaimer states not a government channel',

             'government' in (await page.evaluate(

                 '() => (document.getElementById("aboutDisclaimer")?.textContent || "").toLowerCase()'
    ...
    # v407+: celebrate/kudos on #successGamifyLine; badge progress bar+line under More only.
    first_gamify = await page.evaluate('() => { const el = document.getElementById("successGamifyLine"); return el ? el.textContent.trim() : ""; }')

    first_progress = await page.evaluate('() => { const el = document.getElementById("successProgress"); return el ? el.textContent.trim() : ""; }')

    s.record('RP13', 'Report', 'First report shows gamify + progress', len(first_gamify) > 0 and len(first_progress) > 0,

             f'gamify="{first_gamify[:30]}" progress="{first_progress[:30]}"')
    ...
    norm_gamify = await page.evaluate('() => { const el = document.getElementById("successGamifyLine"); return el && !el.classList.contains("hidden") ? el.textContent.trim() : ""; }')

    norm_progress = await page.evaluate('() => { const el = document.getElementById("successProgress"); return el ? el.textContent.trim() : ""; }')

    celebrate_hidden = await page.evaluate('() => { const el = document.getElementById("successCelebrate"); return !!el && (el.classList.contains("hidden") || el.hasAttribute("hidden")); }')

    s.record('RP14', 'Report', 'Non-milestone report shows gamify kudos (not More celebrate)', len(norm_gamify) > 0 and celebrate_hidden,

             f'gamify="{norm_gamify[:40]}"')
    ...
    # v407+: week streak lives on #successGamifyLine (not duplicated under More).
    streak_visible = await page.evaluate(
        """() => {
          const el = document.getElementById('successGamifyLine');
          if (!el || el.classList.contains('hidden')) return false;
          const txt = (el.textContent || '').toLowerCase();
          return txt.includes('this week') || txt.includes('\u0939\u092b\u093c\u094d\u0924\u0947') || txt.includes('\u0906\u0920\u0935\u0921\u094d\u092f\u093e\u0924') || txt.includes('\u0a85\u0aa0\u0ab5\u0abe\u0aa1\u0abf\u0aaf\u0ac7');
        }"""
    )

    s.record('RW01', 'Rewards', 'Second report shows week streak on gamify line', streak_visible)
