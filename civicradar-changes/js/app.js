...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Single-user confirmation: one neighbour marking a spot fixed resolves it
  // immediately (product decision — trust the reporter). Config-overridable.
  const FIX_CONFIRM_THRESHOLD = SCALE_CFG.fixConfirmThreshold || 1;
  ...
  function checkXpLevelUp(prevXp, newXp) {
    ...
    if (toShow.length && !(certEl && certEl.classList.contains('open'))) {
      setTimeout(() => { celebrate('levelUp'); showCertificateModal(toShow[0]); }, 1400);
    }
  }
  ...
  function focusReportOnMap(reportId) {
    ...
    if (r.lat != null && r.lng != null && map) {
      ...
      if (marker) {
        marker.openPopup();
        // Spring-settle pulse so "your pin" is unmistakable on a busy map.
        if (typeof marker.setRadius === 'function' && !prefersReducedMotion()) {
          const base = marker.getRadius ? marker.getRadius() : 10;
          [0.4, 1.5, 0.85, 1.15, 0.95, 1].forEach((f, i) => {
            setTimeout(() => { try { marker.setRadius(base * f); } catch {} }, i * 70);
          });
        }
      }
    }
  ...
  function handleCommunityAutoResolve(reportId, resolutionSource) {
    ...
    // The flip is FELT: epic confetti + chime — the emotional peak of the loop.
    ...
    if (ownsReport(report)) {
      ...
      // A neighbour (not the reporter) confirmed — the main flow at threshold 1.

      // They're standing at the fixed spot: perfect moment for the after-photo.
  ...
  function confirmFix(reportId, opts) {
    ...
    // At threshold 1 every confirm resolves — the 'resolve' celebration owns
    // that moment; only fire the intermediate burst when it does NOT resolve.
    const willResolveNow = (Number(report.fixConfirmations) || 0) >= FIX_CONFIRM_THRESHOLD;
    if (!willResolveNow) {
      celebrate('fixConfirm');
  ...
  function bindEvents() {
    ...
    if (btnSuccessFilingGuide) {
      ...
      btnSuccessFilingGuide.addEventListener('click', () => {
        ...
        // Land ON the pin they just dropped — dismissing feels like arriving.

        if (reportId) setTimeout(() => focusReportOnMap(reportId), 60);
  ...
  function setButtonLoading(btn, loading, loadingLabel) {
    ...
    // Clearing the submit button's loading state ends a submission — release

    // the re-entrancy lock here (covers all async/geolocation exit paths).

    if (!loading && btn.id === 'btnSubmitReport') submitReport.__inFlight = false;
  ...
  async function submitReport() {
    ...
    // Re-entrancy lock: a rapid second tap can't re-run the async geolocation

    // + duplicate-check path. Released in setButtonLoading(false) + early returns.
  ...
  // ── Celebration system ──────────────────────────────────────────────
  // ONE place to tune celebratory feedback. Tiers map moments to confetti
  // intensity + optional chime. CELEBRATION.enabled=false mutes everything.
  const CELEBRATION = {
    enabled: true,
    tiers: {
      meToo:      { intensity: 'normal',    sound: false },
      report:     { intensity: 'celebrate', sound: true  },
      reportBig:  { intensity: 'epic',      sound: true  },
      fixConfirm: { intensity: 'celebrate', sound: true  },
      resolve:    { intensity: 'epic',      sound: true  },
      levelUp:    { intensity: 'epic',      sound: true  },
      referral:   { intensity: 'celebrate', sound: true  },
    },
  };

  // Synthesized success chime (Web Audio): C5-E5-G5-C6 arpeggio, low gain.
  // No asset, works offline, wrapped so audio can never break a user action.
  let _audioCtx = null;
  ...
  // iOS Safari keeps AudioContext suspended until resumed inside a real user
  // gesture — prime once on first tap so the first chime isn't swallowed.
  ...
  function celebrate(tierName) {
    if (!CELEBRATION.enabled) return;
    const tier = CELEBRATION.tiers[tierName];
    if (!tier) return;
    launchConfetti({ intensity: tier.intensity });
    if (tier.sound && !prefersReducedMotion()) playCelebrationChime();
  }
  ...
  function celebrateReportSubmit(reportCount) {
    ...
    celebrate(isFirst || isMilestone ? 'reportBig' : 'report');
  ...
  function updatePageMetaForReport(report) {
    ...
    const title = `${hazard} — ${ward} | CivicRadar`;
